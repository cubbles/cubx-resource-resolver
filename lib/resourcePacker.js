(function () {
  ...
  class ResourcePacker {
    ...
    async _handleHtmlImport (resource, originUrl, packet) {
      ...
      packet.resources.push(...result.resources);
    ...
    async _resolveHtmlImportDependencies (resource, originUrl) {
      ...
      while ((myArray = re.exec(resource)) !== null) {
        ...
        let re = /.*;(\n)?$/;
        if (!result.htmlImportJavascript.match(re)) {
      ...
      while ((myArray = re.exec(resource)) !== null) {
        ...
        let styleUrl = originUrl.substr(0, originUrl.lastIndexOf('/') + 1);
        await this._handleFonts(styleUrl, res, result);
      }
      resource = resource.replace(re, '');
      // find <link rel="stylesheet"> Tags
    ...
    _handleStylesheet (resource, url, packet) {
      ...
      let styleUrl = url.substr(0, url.lastIndexOf('/') + 1);
      this._handleFonts(styleUrl, resource, packet);
    };

    async _handleFonts (styleUrl, resource, packet) {
      // woff
      let re = /url\("([^"]*)"\)/g;
      let results = [];
      let result;
      while ((result = re.exec(resource)) !== null) {
        results.push(result[1]);
      }

      let promises = results.map(async (result) => {
        if (this.mode === 'dev') {
          console.log('fetch resource:' + result);
        }
        let response = await this._fetchResource(result, styleUrl);
        return {
          path: result,
          resource: response
        };
      ...
      for (let i = 0; i < resultList.length; i++) {
        ...
        packet.resources.push(resultList[ i ]);
      }
    };
